<?php
        ...
        // $GLOBALS['WKPDF_BASE_PATH']=WKPDF_BASE_PATH; // str_replace(str_replace('\\','/',getcwd().'/'),'',dirname(str_replace('\\','/',__FILE__))).'/';
        ...
        class WKPDF {
                ...
                public function output($mode,$file){
                        if( $this->debug ) echo "WKPDF->output($mode,$file)\n";
                        switch($mode){
                                ...
                                case self::$PDF_SAVEFILE:
                                        if( $this->debug ) echo "Writing ".strlen($this->pdf)." bytes to $file\n";
