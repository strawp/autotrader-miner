<?php
  ...
  class CshField extends Field{
    function CshField( $fieldname, $options="" ){
      $this->Field( $fieldname, $options );
      $this->length = 20;
      $this->defaultorderdir = "desc";
      if( $this->type == "csh" ) $this->init();
    }
    ...
    */
    function getDataType(){
      // return "decimal(".$this->length.",2)";
      return "bigint(".$this->length.")";
    }
    
    /**
    * Switch cash fields from int (pennies) to decimal (pounds)
    * @param string 
    */
    /*
    function afterModifyColumn( $previoustype ){
      switch( $previoustype ){
        case "bigint(20)":
          $sql = "UPDATE ".$this->parent_tablename." SET ".$this->columnname." = ".$this->columnname."/100";
          $db = new DB();
          echo $sql."\n";
          $db->query( $sql );
          echo $db->error;
    ...
    function set( $value, $is_search=false ){
      ...
      if( $is_search ){
        ...
        if( !is_array( $value ) ){
          ...
          $this->value = round( $value * 100 );
        }else{
          $value[0] = round( preg_replace( "/[^-\.0-9]/", "", $value[0] ) * 100 );
          $value[1] = round( preg_replace( "/[^-\.0-9]/", "", $value[1] ) * 100 );
    ...
    */
    function toString( $aData=array() ){
      ...
      addLogMessage( "End", $this->name."->toString()" );
      return number_format( $this->value / 100, $this->sigfigures );
    }
    ...
    */
    function getStatsSelectStatement(){
      return array(
        "FORMAT( SUM( ".$this->parent_tablename.".".$this->columnname." )/100, 2 ) as ".$this->columnname."_sum",
        "COUNT( * ) as ".$this->columnname."_count",
        "FORMAT( AVG( ".$this->parent_tablename.".".$this->columnname." )/100, 2 ) as ".$this->columnname."_average",
        "FORMAT( STD( ".$this->parent_tablename.".".$this->columnname." )/100, 2 ) as ".$this->columnname."_deviation",
        "FORMAT( MAX( ".$this->parent_tablename.".".$this->columnname." )/100, 2 ) as ".$this->columnname."_max",
        "FORMAT( MIN( ".$this->parent_tablename.".".$this->columnname." )/100, 2 ) as ".$this->columnname."_min"
      );
    }
