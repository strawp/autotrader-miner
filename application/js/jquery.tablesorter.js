...
(function($) {
	...
	$.fn.tableSorter = function(o) {
		...
		var defaults =  {
			...
			lockedSortDir: false,
			enableResize: false,
			dateFormat: 'dd mmm yyyy' /** us default, uk dd/mm/yyyy */
		};
	...
	$.tableSorter.parsers.shortDate = {
		id: 'shortDate',
		is: function(s) {
			return s.match(new RegExp(/\d{1,2}[ \/-](\d{1,2}|[a-z]{3})[ \/-]\d{2,4}/i));
		},
		format: function(s,defaults) {
			s = s.replace(new RegExp(/-/g),'/');
			if(defaults.dateFormat == "dd mmm yyyy" ) {
        // Short string date, reformat
        dte = new Date();
        dte.setTime(Date.parse(s));
        s = dte.getYear() + "/" + dte.getMonth() + "/" + dte.getDate();
			}else if(defaults.dateFormat == "mm/dd/yyyy" || defaults.dateFormat == "mm-dd-yyyy") {
